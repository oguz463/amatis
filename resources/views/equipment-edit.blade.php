      ...
      <div class="w-full">
          <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg mb-32">
              ...
              <header class="font-semibold bg-gray-200 text-gray-700 py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md">
                  <strong>Edit:</strong> {{ $equipment->name }}
              </header>
              ...
              <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8 mb-6" method="POST"
                  ...
                  <div class="flex flex-wrap">
                      ...
                      <input id="name" type="text" class="form-input w-full @error('name')  border-red-500 @enderror"
                          name="name" value="{{ old('name') ? old('name') : $equipment->name}}">
                  ...
                  <div class="flex flex-wrap">
                    <div class="p-6 shadow rounded border">
                      ...
                      @foreach ($equipment->info as $value)
                        <input type="text"
                        class="form-input w-full mb-2" name="info[]" value="{{$value}}"
                        >
                      @endforeach
                      </div>
                  ...
                  <div class="flex flex-wrap space-x-2 items-center">
                      ...
                      <input id="date" type="date"
                          ...
                          value="{{ old('date') ? old('date') : $equipment->date }}">
                      @error('date')
                      <p class="text-red-500 text-xs italic mt-4">
                          {{ $message }}
                      </p>
                      ...
                      <input id="price" type="text"
                          ...
                          value="{{ old('price') ? old('price') : $equipment->price }}">
                      @error('price')
                      <p class="text-red-500 text-xs italic mt-4">
                          {{ $message }}
                      </p>
                      ...
                      <input id="rate" type="text"
                          ...
                          value="{{ old('rate') ? old('rate') : $equipment->rate }}">
              ...
              <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8 mb-6" method="POST" action="{{ route('equipment.delete', $equipment->id) }}" onsubmit="return confirm('Do you really want to delete?');">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
                  class="w-full select-none font-bold whitespace-no-wrap p-3 rounded-lg text-base leading-normal no-underline text-gray-100 bg-red-600 hover:bg-red-900 sm:py-4">
                  Delete
                </button>
              </form>
